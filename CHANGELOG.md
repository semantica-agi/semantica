...
- **Fix: Knowledge Explorer — Vite dev-server proxy and README corrected** (`explorer/vite.config.ts`, `explorer/README.md`, `README.md`, by @KaifAhmad1):
  - Corrected the Vite dev-server proxy targets to `http://127.0.0.1:8000` / `ws://127.0.0.1:8000` — the default port that `python -m semantica.explorer` binds on. Both the `/api` HTTP proxy and the `/ws` WebSocket proxy are updated. Without this, all frontend API calls and live-update WebSocket connections fail with connection-refused errors during local development.
  - Updated `explorer/README.md` and the root `README.md` Explorer section: replaced the incorrect `python -m semantica.server` command with the correct `python -m semantica.explorer --graph <file>` invocation, documented all CLI flags (`--graph`, `--port`, `--host`, `--no-browser`), added the graph JSON file format, expanded the workspace tab descriptions to cover all tabs (Graph Studio, Timeline, Decisions, Enrich, KG Overview, Vocabulary, Ontology, Registry), and added a troubleshooting entry for port conflicts.

- **Fix: Knowledge Explorer — path finding and provenance graph traversal** (by @KaifAhmad1):
  - **Path finding now uses a bidirectional graph** (`semantica/explorer/routes/graph.py`): The path-finding route built a `networkx.DiGraph` from `build_graph_dict()` output, meaning paths could only be found in the direction edges were originally stored — a `GET /api/graph/node/fastapi/path?target=alice` would fail even though the two nodes were connected via an intermediate hop. Changed to `nx.Graph()` so paths are discovered in both directions, which is the correct behaviour for a knowledge graph explorer where users query connectivity, not edge orientation.
  - **Provenance traversal now uses a directed graph with undirected ego expansion** (`semantica/explorer/routes/provenance.py`): The internal provenance subgraph is built as `nx.DiGraph()` to preserve edge direction (upstream vs downstream lineage), while `nx.ego_graph(..., undirected=True)` ensures that nodes with incoming edges to the queried node (e.g. persons, decisions, events) are included in the neighbourhood view alongside outgoing neighbours. Previously `undirected=False` caused only outgoing-edge neighbours to appear, hiding all upstream provenance context.
  - All 85 explorer tests continue to pass after both changes.
