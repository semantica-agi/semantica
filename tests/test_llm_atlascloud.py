"""Tests for the AtlasCloud LLM provider wrapper (semantica.llms.AtlasCloud)."""
...
def test_construction_stores_model_and_api_key():
    llm = AtlasCloud(model="deepseek-ai/deepseek-v3.2", api_key="fake-key")
...
def test_generate_raises_clear_error_when_unavailable(monkeypatch):
    monkeypatch.delenv("ATLASCLOUD_API_KEY", raising=False)
    llm = AtlasCloud(api_key=None)
    with pytest.raises(ProcessingError, match="Atlas Cloud provider not available"):
        llm.generate("hello")


def test_generate_forwards_to_the_real_provider_when_available():
    llm = AtlasCloud(api_key="fake-key")
