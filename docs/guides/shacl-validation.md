...
- **Running range checks under RDFS inference**: `rdfs:range` is an *entailment* rule, not a constraint. With `inference="rdfs"`, every object of the property is inferred into the range class, so `sh:class` range checks on it can never fail and the report says `conforms: True` on data that does not conform. Validate those shapes with `inference="none"`, or omit `rdfs:range` for properties you want SHACL to police — the trade-off is that `sh:targetClass` stops reaching subclasses. Turning inference *on* reads like making validation stricter; for range constraints it does the opposite. Note that `sh:node` over a shape requiring an explicit `rdf:type` does not rescue it either: the entailment materialises the type as a real triple, so nothing downstream can tell a declared type from an inferred one.
